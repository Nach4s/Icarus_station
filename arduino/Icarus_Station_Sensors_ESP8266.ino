/*
 * Icarus Station - Environmental Monitoring System with WiFi (ESP8266 Version)
 * 
 * Sensors:
 * - Temperature & Humidity: AHT20 (I2C) or DHT22 (Digital pin 2)
 * - CO2: MH-Z19B (SoftwareSerial: RX=3, TX=4)
 * - Oxygen (O2): ZE07-O2 (SoftwareSerial: RX=5, TX=6)
 * - Carbon Monoxide (CO): MQ-7 (Analog pin A0)
 * 
 * Display: OLED SSD1306 0.96" (I2C: SDA=D2, SCL=D1)
 * 
 * WiFi: Built-in ESP8266 WiFi
 * 
 * Libraries required:
 ...
 * - ESP8266WiFi
 * - ESP8266HTTPClient
 * - ArduinoJson (optional, for better JSON handling)
 */

#include <Adafruit_AHTX0.h>
// #include <DHT.h>  // Uncomment if using DHT22 instead of AHT20
...
// ==================== PIN DEFINITIONS ====================
// For ESP8266: SDA=D2, SCL=D1 (default I2C pins)
...
#define MH_Z19_RX 12  // D6 on ESP8266
#define MH_Z19_TX 13  // D7 on ESP8266

// ZE07-O2 Oxygen Sensor (SoftwareSerial)
#define ZE07_O2_RX 14  // D5 on ESP8266
#define ZE07_O2_TX 16  // D0 on ESP8266
...
void setup() {
  ...
  if (!aht.begin()) {
    ...
    // Don't block, continue with other sensors
  } else {
...
void sendDataToServer(SensorData data) {
  ...
  if (httpResponseCode > 0) {
    ...
    // Update display to show connection error
    display.setCursor(0, 56);
...
int readCO() {
  ...
  float voltage = (sensorValue / 1024.0) * 3.3;  // ESP8266 uses 3.3V
